...
const App = {
  ...
  // Scroll to Site Card with smooth animation and highlight
  scrollToSite(siteId) {
    ...
    document.querySelectorAll('.geo-step-pill').forEach(pill => {
      if (pill.dataset.target === siteId) pill.classList.add('active');
      else pill.classList.remove('active');
    });
  },

  // REDESIGNED Geographic Multi-Site Rollout Plan View (Mobile-First & Touch Optimized)
      ...
      <div class="geo-deployment-portal">
        ...
        <div class="mb-5">
          <div class="flex items-center gap-2 mb-2 flex-wrap">
            <span class="px-2.5 py-0.5 rounded text-xs font-bold bg-cyan-950 text-cyan-300 border border-cyan-800">خطة التوزيع الميداني المؤسسي</span>
            <span class="text-xs text-slate-400 font-mono">5 مواقع جغرافية + 48 جهاز + ربط هوائي 18km</span>
          </div>
          <h2 class="text-xl sm:text-2xl font-black text-white mb-1">${geographicPlan.title}</h2>
          <p class="text-xs text-slate-300">${geographicPlan.description}</p>
        </div>

        <!-- Touch-friendly Horizontal Stepper / Quick Site Selector -->
        <div class="geo-stepper-bar">
          <button class="geo-step-pill active" data-target="master-tower-section" onclick="App.scrollToSite('master-tower-section')">
            <span>🗼</span> البرج الرئيسي (HQ Tower)
          </button>
          <button class="geo-step-pill" data-target="site-site-admin" onclick="App.scrollToSite('site-site-admin')">
            <span>🏢</span> 1. فرع الإدارة (4.2km)
          </button>
          <button class="geo-step-pill" data-target="site-site-warehouse" onclick="App.scrollToSite('site-site-warehouse')">
            <span>🏭</span> 2. المستودعات (11.8km)
          </button>
          <button class="geo-step-pill" data-target="site-site-plant" onclick="App.scrollToSite('site-site-plant')">
            <span>⚙️</span> 3. مصنع الإنتاج (18.5km)
          </button>
          <button class="geo-step-pill" data-target="site-site-retail" onclick="App.scrollToSite('site-site-retail')">
            <span>💳</span> 4. نقاط البيع (2.8km)
          </button>
          <button class="geo-step-pill" data-target="site-site-teleworkers" onclick="App.scrollToSite('site-site-teleworkers')">
            <span>🔒</span> 5. العمالة الميدانية (VPN)
          </button>
        </div>

        <!-- Master Base Station Card -->
        <div id="master-tower-section" class="master-tower-hero">
          <div class="flex justify-between items-start flex-wrap gap-3 mb-4">
            <div>
              <span class="text-3xs font-bold text-amber-400 uppercase font-mono tracking-wider block mb-1">
                نواة المنظومة المركزية (MASTER BASE STATION)
              </span>
            ...
            <div class="text-left direction-ltr">
              <span class="px-2.5 py-1 bg-amber-950/90 border border-amber-700 text-amber-300 text-xs font-mono rounded-lg block mb-1">
                ${masterTower.coordinates}
              </span>
              <span class="text-3xs text-slate-300 font-mono">Backbone: ${masterTower.backboneInternet}</span>
            </div>
          </div>

          <!-- Master Tower Real Photos Gallery Preview (Responsive Grid) -->
          <div class="mb-4 p-3.5 bg-slate-950/90 rounded-xl border border-slate-800">
            <div class="flex items-center justify-between mb-2.5">
              ...
              <span class="text-3xs text-slate-400">انقر على أي جهاز للمواصفات</span>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2.5">
              <div class="p-2.5 bg-slate-900 rounded-xl border border-slate-800 text-center cursor-pointer group hover:border-cyan-500 transition-all flex flex-col justify-between" onclick="App.openProductModal('ccr2004')" title="انقر لعرض المواصفات">
                <div class="h-24 w-full flex items-center justify-center mb-1.5 bg-slate-950/70 rounded-lg p-1">
                ...
                <span class="text-xs text-slate-200 font-bold block truncate">CCR2004 10G</span>
                <span class="text-3xs text-cyan-400 block font-mono">MikroTik Core</span>
                <span class="text-3xs text-slate-400 group-hover:text-cyan-300 font-bold mt-1">🔍 فحص</span>
              </div>

              <div class="p-2.5 bg-slate-900 rounded-xl border border-slate-800 text-center cursor-pointer group hover:border-cyan-500 transition-all flex flex-col justify-between" onclick="App.openProductModal('crs328')" title="انقر لعرض المواصفات">
                <div class="h-24 w-full flex items-center justify-center mb-1.5 bg-slate-950/70 rounded-lg p-1">
              ...
              <div class="p-2.5 bg-slate-900 rounded-xl border border-slate-800 text-center cursor-pointer group hover:border-cyan-500 transition-all flex flex-col justify-between" onclick="App.openProductModal('rocket-prism')" title="انقر لعرض المواصفات">
                <div class="h-24 w-full flex items-center justify-center mb-1.5 bg-slate-950/70 rounded-lg p-1">
              ...
              <div class="p-2.5 bg-slate-900 rounded-xl border border-slate-800 text-center cursor-pointer group hover:border-cyan-500 transition-all flex flex-col justify-between" onclick="App.openProductModal('powerbeam')" title="انقر لعرض المواصفات">
                <div class="h-24 w-full flex items-center justify-center mb-1.5 bg-slate-950/70 rounded-lg p-1">
                  <img src="assets/images/powerbeam.jpg" alt="PowerBeam" class="max-h-full max-w-full object-contain group-hover:scale-105 transition-transform">
                </div>
              ...
              <div class="p-2.5 bg-slate-900 rounded-xl border border-slate-800 text-center cursor-pointer group hover:border-cyan-500 transition-all flex flex-col justify-between" onclick="App.openProductModal('cable-stp')" title="انقر لعرض المواصفات">
                <div class="h-24 w-full flex items-center justify-center mb-1.5 bg-slate-950/70 rounded-lg p-1">
          ...
          <!-- Master Tower Specs Grid -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            <div class="p-3.5 bg-slate-950/80 rounded-xl border border-slate-800">
              <strong class="text-amber-300 block mb-1.5">عتاد المقر الرئيسي وأجهزة البث:</strong>
            ...
            <div class="p-3.5 bg-slate-950/80 rounded-xl border border-slate-800">
              <strong class="text-cyan-300 block mb-1.5">توزيع قنوات التردد لمنع التداخل (RF Channels Allocation):</strong>
              <div class="space-y-1.5 text-2xs text-slate-300">
        ...
        <div class="sites-list-container">
          ${sites.map((site, index) => {
            ...
            return `
              <div id="site-${site.id}" class="site-card-v2">
                <!-- Site Header Row -->
                <div class="site-header-row">
                  <div class="site-title-box">
                    <div class="site-badge-num">${index + 1}</div>
                    <div>
                      <h4 class="site-name-text">${site.name}</h4>
                      <span class="text-2xs text-amber-400 font-mono">المسافة: ${site.distance} • ${site.losStatus}</span>
                    </div>
                  </div>

                  <div class="site-meta-pills">
                    <span class="site-meta-pill signal">
                      <span>📶 الإشارة:</span> <strong>${site.targetSignal}</strong>
                    </span>
                    <span class="site-meta-pill bandwidth">
                      <span>⚡ السعة:</span> <strong>${site.bandwidthAllocated}</strong>
                    </span>
                  </div>
                </div>
                ...
                <div class="site-stats-row">
                  <span class="font-bold text-slate-300">👥 الأجهزة والمستخدمين بالموقع:</span>
                  <span class="text-white">${site.userCount}</span>
                </div>

                <!-- 3 Hardware Columns (Responsive) -->
                <div class="site-hw-grid">
                  <!-- Column 1: CPE Receiver -->
                  <div class="site-hw-col">
                    <div>
                      <div class="flex items-center justify-between text-amber-400 font-bold text-2xs mb-2">
                        <span>📡 لاقط الاستقبال الهوائي (CPE)</span>
                        <span class="text-3xs text-emerald-400">صورة معتمدة</span>
                      </div>
                      <div class="site-hw-photo-box" onclick="App.openProductModal('${siteConfig.cpeId}')" title="انقر لعرض المواصفات">
                        <img src="${siteConfig.cpeImg}" alt="CPE">
                        <div class="site-hw-inspect-badge">🔍 فحص</div>
                      </div>
                      <strong class="text-white text-xs block mb-0.5">${site.cpeReceiver.brand} - ${site.cpeReceiver.model}</strong>
                      <span class="text-3xs text-slate-400 block">${site.cpeReceiver.type} (${site.cpeReceiver.gain})</span>
                    </div>
                    <div class="mt-2 pt-2 border-t border-slate-900 flex justify-between items-center text-3xs">
                      <span class="text-cyan-300 font-mono">${site.cpeReceiver.mount || 'تثبيت ماست'}</span>
                      <button class="text-amber-400 font-bold hover:underline" onclick="App.openProductModal('${siteConfig.cpeId}')">المواصفات ←</button>
                    </div>
                  </div>
                  ...
                  <div class="site-hw-col">
                    <div>
                      <div class="flex items-center justify-between text-cyan-400 font-bold text-2xs mb-2">
                        <span>🔀 سويتش التوزيع والـ PoE</span>
                        <span class="text-3xs text-emerald-400">صورة معتمدة</span>
                      </div>
                      <div class="site-hw-photo-box" onclick="App.openProductModal('${siteConfig.swId}')" title="انقر لعرض المواصفات">
                        <img src="${siteConfig.swImg}" alt="Switch">
                        <div class="site-hw-inspect-badge">🔍 فحص</div>
                      </div>
                      <strong class="text-white text-xs block mb-0.5">${site.distributionSwitch.brand}</strong>
                      <span class="text-3xs text-slate-300 block truncate">${site.distributionSwitch.model}</span>
                    </div>
                    <div class="mt-2 pt-2 border-t border-slate-900 flex justify-between items-center text-3xs">
                      <span class="text-slate-400 truncate max-w-[65%]">${site.distributionSwitch.role}</span>
                      <button class="text-cyan-400 font-bold hover:underline" onclick="App.openProductModal('${siteConfig.swId}')">المواصفات ←</button>
                    </div>
                  </div>
                  ...
                  <div class="site-hw-col">
                    <div>
                      <div class="flex items-center gap-1 text-emerald-400 font-bold text-2xs mb-2">
                        <span>📶</span> موزع الواي فاي والمودم الداخلي
                      </div>
                      ${site.localAPs && site.localAPs.length > 0 ? site.localAPs.map(ap => `
                        <div class="mb-2 pb-2 border-b border-slate-900 last:border-0 last:mb-0 last:pb-0">
                          <strong class="text-slate-200 text-2xs block">${ap.brand} ${ap.model}</strong>
                          <span class="text-3xs text-slate-400 block">${ap.role}</span>
                          <span class="text-3xs text-amber-400 font-mono block mt-0.5">${ap.mode}</span>
                        </div>
                      `).join('') : `
                        <div class="text-2xs text-slate-300 leading-relaxed">
                          اتصال أنفاق WireGuard المشفرة للموظفين الميدانيين مع عزل تام وتشفير ChaCha20-Poly1305.
                        </div>
                      `}
                    </div>
                    <div class="mt-2 pt-2 border-t border-slate-900 text-3xs text-slate-400">
                      <span>إدارة العناوين: DHCP المركزي من المقر</span>
                    </div>
                  </div>
                </div>

                <!-- Footer Wiring & Subnet Strip -->
                <div class="site-wiring-strip">
                  <div>
                    <span class="text-slate-400">نطاق الشبكة والـ VLAN:</span>
                    <strong class="text-cyan-400 font-mono mr-1">${site.ipSubnet}</strong>
                  </div>
                  <div>
                    <span class="text-slate-400">التوصيل والتأريض:</span>
